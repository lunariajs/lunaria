...
2. Add the [`dashboard.description`](/reference/configuration/#dashboarddescription) option to your Lunaria config:
    ...
    ```js title="lunaria.config.mjs" add={6}
    import { defineConfig } from '@lunariajs/core/config';
...
```js title="lunaria.config.mjs" add={5}
import { defineConfig } from '@lunariajs/core/config';
...
```js title="lunaria.config.mjs" add={6-15}
import { defineConfig } from '@lunariajs/core/config';
...
:::tip
Check out the Lunaria repository to see [all the available CSS custom properties set by Lunaria and its implementation](https://github.com/lunariajs/lunaria/blob/main/packages/core/src/dashboard/styles.ts). 
:::
...
```js title="lunaria.config.mjs" add={6-8}
import { defineConfig } from '@lunariajs/core/config';
...
<Steps>
1. Add the [`renderer`](/reference/configuration/#renderer) configuration property to your Lunaria config:
...
:::tip
If you prefer to keep your renderer configuration in a separate file, write it using the [`defineRendererConfig()` helper](/reference/renderer/#definerendererconfig) and import it into the `renderer` property of your main Lunaria configuration.
:::
...
```js title="example-component.mjs"
...
```js title="interpolating-value.mjs"
import { html } from '@lunariajs/core';
...
```js title="interpolating-list.mjs"
import { html } from '@lunariajs/core';
...
1. Create a new component using the `html` tagged template. In this case, a `"robots"` [meta tag](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/meta) with the `"noindex"` content to avoid indexing the dashboard in search engines:
    ...
    ```js title="lunaria.config.mjs" add={1, 4}
    import { html } from '@lunariajs/core';
    import { defineConfig } from '@lunariajs/core/config';
...
2. Add the created component to one of the available slots in the [`slots` property](/reference/renderer/#slots). Here, the `head` slot will be used to insert the component into the dashboard's [`<head>` element](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/head):
    ...
    ```js title="lunaria.config.mjs" add={8-10}
    import { html } from '@lunariajs/core';
    import { defineConfig } from '@lunariajs/core/config';
...
<Steps>
1. Create a new component using the `html` tagged template. In this case, the component will override the dashboard's body and show the source path of each tracked file:
    
    ```js title="lunaria.config.mjs" add={1, 4}
    import { html } from '@lunariajs/core';
    import { defineConfig } from '@lunariajs/core/config';

    const NewBody = (config, status) => html`<ul>${status.map((entry) => html`<li>${entry.source.path}</li>`)}</ul>`;
