...
export const UnsupportedDictionaryFileFormat = {
	...
	message: (file: string) =>
		`The file \`${file}\` has an unsupported file format. Dictionaries can be JSON, YAML, gettext PO/POT, or JavaScript/TypeScript modules. Use one of these file formats or instead track this file without key completion checking by setting it to \`type: "universal"\` instead.`,
} satisfies ErrorContext;
